...
> 这是 `humanizer` skill 的随包副本，供 C8 在宿主没装 humanizer 时使用。
> 来源：翻译自 blader/humanizer，参考 hardikpandya/stop-slop，MIT 许可（见同目录 `humanizer-zh.LICENSE.txt`）。
> 宿主装了 humanizer skill 时优先调那个，别读这份，它不会随上游更新。
